...
class ZyplineREST{
	...
	function add_pair( $index, $file ){
		error_log('library->add_pair');
		...
		$params = array( 'index' => $index, 'url' => $url, 'uploadedfile' => $file_str);
	...
	private function call_uri( $data, $uri, $verb ){
		...
		//array_push($header_str, "Content-type: multipart/form-data");
		//$header_str = array("Content-type: multipart/form-data");
	...
	private function create_hash( $data, $api_key ){
		...
		error_log( 'Library Hash:  ' . " $str - $api_key " . hash_hmac('sha1', $str, $api_key) );
